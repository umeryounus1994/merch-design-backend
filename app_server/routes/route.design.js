...
router.get("/deleteActualImages/:designId/:actualImageId", function (req, res) {
  designs.deleteActualImage(
    req.params.designId,
    req.params.actualImageId,
    function (err, result) {
      if (err)
        return res.json({
          Message: "Error in Connecting to DB",
...
router.post("/filerDesign", function (req, res) {
  ...
  designs.filerDesign(body, function (err, result) {
    ...
    result.forEach((el) => {
      if (
        parseInt(el.designPrice) >= body.minPrice &&
        parseInt(el.designPrice) <= body.maxPrice
      ) {
        desigData.push(el);
      }
    });
...
router.get("/downloadFiles/:filename", function (req, res) {
